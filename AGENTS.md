...
- Treat plan-state sync as mandatory: before every `final` response, ensure any active `plan` has no `in_progress` steps; if the task is too small to benefit from a plan, prefer not to create one.
